...
public class FileController {
    ...
    public Result<PageVO<FileInfoVO>> listFiles(
            @Parameter(description = "租户ID")
            @RequestParam String tenantId,
            @Parameter(description = "业务标签列表，多个标签用逗号分隔")
            @RequestParam(required = false) java.util.List<String> bizTags,
            @Parameter(description = "页码，从1开始")
            @RequestParam(defaultValue = "1") Integer page,
            @Parameter(description = "每页大小")
            @RequestParam(defaultValue = "20") Integer pageSize) {
        log.info("Received file list request: tenantId={}, bizTags={}, page={}, pageSize={}", 
                tenantId, bizTags, page, pageSize);
        
        // 构建查询参数
        FileQueryDTO queryDTO = new FileQueryDTO();
        queryDTO.setTenantId(tenantId);
        queryDTO.setBizTags(bizTags);
        
        PageDTO<FileQueryDTO> pageDTO = new PageDTO<>();
        pageDTO.setPageNum(page);
        pageDTO.setPageSize(pageSize);
        pageDTO.setParams(queryDTO);
        
        PageVO<FileInfoVO> result = fileService.queryFiles(pageDTO);
     ...
     */
    @PostMapping("/query")
    @Operation(summary = "查询文件列表（高级）", description = "分页查询文件，支持更多筛选条件")
    @SiaeAuthorize("hasAuthority('" + MEDIA_FILE_QUERY + "')")
    public Result<PageVO<FileInfoVO>> queryFiles(
            @Parameter(description = "分页查询参数")
            @Valid @RequestBody PageDTO<FileQueryDTO> pageDTO) {
    ...
    public Result<FileInfoVO> updateFile(
            @Parameter(description = "文件ID")
            @PathVariable String fileId,
            @Valid @RequestBody FileUpdateDTO request) {
        log.info("Received update file request: fileId={}", fileId);
        FileInfoVO response = fileService.updateFile(fileId, request);
        return Result.success(response);
    }
    ...
    @SiaeAuthorize("hasAuthority('" + MEDIA_FILE_MANAGE + "')")
    public Result<FileInfoVO> restoreFile(
            @Parameter(description = "文件ID")
            @PathVariable String fileId) {
        log.info("Received restore file request: fileId={}", fileId);
        FileInfoVO response = fileService.restoreFile(fileId);
        return Result.success(response);
    }
