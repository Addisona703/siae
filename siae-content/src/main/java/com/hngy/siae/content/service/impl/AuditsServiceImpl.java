...
public class AuditsServiceImpl
    ...
    public Long submitAudit(AuditDTO auditDTO) {
        ...
        AssertUtil.isFalse(exists, "审核请求已存在，不能重复请求");

        // 2. 发起请求
        Audit audit = BeanUtil.copyProperties(auditDTO, Audit.class);
        AssertUtil.isTrue(this.save(audit), "提交审核请求失败");
        return audit.getId();
    }
    ...
    public Result<Void> handleAudit(Long id, AuditDTO auditDTO) {
        ...
        AssertUtil.notNull(audit, "这条审核申请不存在");
        AssertUtil.isTrue(audit.getAuditStatus() == AuditStatusEnum.PENDING, "审核已被处理，请勿重复审核");
        ...
        audit.setId(id);
        AssertUtil.isTrue(this.updateById(audit), "处理审核失败");
    ...
    public Result<AuditVO> getAuditRecord(Long targetId, TypeEnum targetType) {
        ...
        AssertUtil.notNull(audit, "未找到相关审核记录");
