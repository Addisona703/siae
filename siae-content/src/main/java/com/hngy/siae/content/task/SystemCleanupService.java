...
//                            .lt(Content::getUpdateTime, LocalDateTime.now().minusDays(30))
            ...
            AssertUtil.isTrue(auditRemoved, "批量删除审核记录失败");
            ...
            if (count > 0) {
                ...
                AssertUtil.isTrue(tagsRemoved, "批量删除标签关系失败");
            }
            ...
            typeIdMap.forEach((type, idList) -> strategyContext.tryGetStrategy(type)
                    .ifPresent(strategy -> {
                        ...
                        AssertUtil.isTrue(deleted, "策略批量删除详情失败，类型：" + type);
                    }));

            // 批量删除主内容
            boolean contentRemoved = contentService.removeByIds(contentIds);
            AssertUtil.isTrue(contentRemoved, "批量删除主内容失败");
...
//                            .lt(Content::getUpdateTime, LocalDateTime.now().minusDays(30))
            ...
            if (!success) {
                ...
                AssertUtil.isFalse(false, "分页批量更新回收站内容状态失败，页码：" + page);
            }
    ...
    public void cleanDeletedCategory() {
        ...
        AssertUtil.isTrue(removed, "批量删除分类失败");
