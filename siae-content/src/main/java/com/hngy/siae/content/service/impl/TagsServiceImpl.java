...
public class TagsServiceImpl
    ...
    public Result<TagVO> createTag(TagDTO tagDTO) {
        ...
        boolean exists = lambdaQuery()
                ...
                .exists();
        AssertUtil.isFalse(exists, "标签已存在");

        // 插入标签
        Tag tag = BeanUtil.copyProperties(tagDTO, Tag.class);
        AssertUtil.isTrue(this.save(tag), "标签保存失败，请重试");
    ...
    public Result<TagVO> updateTag(Long id, TagDTO tagDTO) {
        ...
        boolean exists = lambdaQuery()
                ...
                .ne(Tag::getId, id)
                .exists();
        AssertUtil.isFalse(exists, "标签已存在，不要重复更新");

        // 拷贝属性并更新
        Tag tag = BeanUtil.copyProperties(tagDTO, Tag.class);
        tag.setId(id);
        AssertUtil.isTrue(this.updateById(tag), "更新标签失败");
    ...
    public Result<Void> deleteTag(Long id) {
        ...
        AssertUtil.notNull(existingTag, "标签不存在");

        // 删除标签-内容关联表数据
        tagRelationService.deleteRelations(id);

        // 删除标签
        AssertUtil.isTrue(this.removeById(id), "删除标签失败");
