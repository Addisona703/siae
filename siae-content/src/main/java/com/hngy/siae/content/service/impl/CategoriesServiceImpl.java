...
public class CategoriesServiceImpl
    ...
    public Result<CategoryVO> createCategory(CategoryDTO categoryDTO) {
        ...
        boolean exists = lambdaQuery()
                ...
                .exists();
        AssertUtil.isFalse(exists, "分类名称已存在");

        // 插入分类
        Category category = BeanUtil.copyProperties(categoryDTO, Category.class);
        AssertUtil.isTrue(baseMapper.insert(category) > 0, "新增分类失败");
    ...
    public Result<CategoryVO> updateCategory(CategoryDTO categoryDTO) {
        ...
        AssertUtil.notNull(existingCategory, "分类不存在");
        ...
        boolean exists = lambdaQuery()
                ...
                .exists();
        AssertUtil.isFalse(exists, "分类名称或编码已存在");

        // 存在则更新分类
        Category category = BeanUtil.copyProperties(categoryDTO, Category.class);
        AssertUtil.isTrue(this.updateById(category), "更新分类失败");
    ...
    public Result<Void> deleteCategory(Integer categoryId) {
        ...
        boolean hasChildren = lambdaQuery()
                ...
                .exists();
        AssertUtil.isFalse(hasChildren, "该分类下存在子分类，无法删除");

        // 逻辑删除
        Category category = new Category();
        category.setId(existingCategory.getId());
        category.setStatus(CategoryStatusEnum.DELETED);
        AssertUtil.isTrue(this.updateById(category), "删除分类失败");
    ...
    public Result<CategoryVO> queryCategory(long categoryId) {
        ...
        AssertUtil.isFalse(category == null
    ...
    public Result<Void> updateStatus(long categoryId, boolean enable) {
        Category category = this.getById(categoryId);
        AssertUtil.notNull(category, "该分类不存在");
        ...
        AssertUtil.isTrue(this.updateById(category), "分类状态更新失败");
        return Result.success();
    }
}