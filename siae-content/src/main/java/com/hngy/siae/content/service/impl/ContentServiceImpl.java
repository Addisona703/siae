...
public class ContentServiceImpl
    ...
    public Content createContent(ContentDTO dto) {
        // 1. 检查重复
        AssertUtil.isFalse(this.lambdaQuery()
        ...
        AssertUtil.notNull(type, "不支持的类型：" + dto.getType());
        ...
        // 自动回填主键，id
        AssertUtil.isTrue(this.save(content), "内容插入失败");
    ...
    public Content updateContent(ContentDTO contentDTO) {
        // 1. 校验内容是否存在
        AssertUtil.isTrue(this.lambdaQuery()
        ...
        Optional.ofNullable(contentDTO.getType())
                .map(typeStr -> {
                    ...
                    AssertUtil.notNull(type, "不支持的内容类型: " + typeStr);
                    return type;
                })
                .ifPresent(content::setType);

        // 3. 更新数据库
        AssertUtil.isTrue(this.updateById(content), "内容更新失败");
    ...
    public Result<Void> deleteContent(Integer id, Integer isTrash) {
        // 1. 参数校验
        AssertUtil.isTrue(isTrash == 0 || isTrash == 1, "无效操作");

        // 2. 查询内容是否存在
        Content content = this.getById(id);
        AssertUtil.notNull(content, "内容不存在，无法删除");

        // 3. 删除
        if (isTrash == 1) {
            // 放入回收站（软删除）
            content.setStatus(ContentStatusEnum.TRASH);
            AssertUtil.isTrue(this.updateById(content), "放入回收站操作失败");
        } else {
            // 不放入回收站（伪硬删除，通过定时任务实现硬删除）
            content.setStatus(ContentStatusEnum.DELETED);
            AssertUtil.isTrue(this.updateById(content), "永久删除操作失败");
        }
    ...
    @Override
    public Result<PageVO<ContentVO<EmptyDetailVO>>> getContentPage(ContentPageDTO dto) {
        // 创建分页参数
        Page<Content> page = new Page<>(dto.getPage(), dto.getPageSize());
        // 查询数据
        List<Content> list = contentMapper.selectByCondition(page, dto);
        AssertUtil.notNull(list, "这里没有任何内容");
        // 查询总数
        long total = contentMapper.countByCondition(dto);
