...
public class CommentsServiceImpl
    ...
    public Comment createComment(Long contentId, CommentDTO commentDTO) {
        ...
        AssertUtil.isTrue(this.save(comment), "评论创建失败");
        return comment;
    }
    ...
    public Result<CommentVO> updateComment(Long commentId, CommentDTO commentDTO) {
        ...
        AssertUtil.notNull(comment, "该评论不存在");

        comment.setContent(commentDTO.getContent());
        AssertUtil.isTrue(this.updateById(comment), "评论更新失败");
    ...
    public Result<Void> deleteComment(Long id) {
        AssertUtil.notNull(this.getById(id), "评论不存在");
        AssertUtil.isTrue(this.removeById(id), "删除评论失败");
